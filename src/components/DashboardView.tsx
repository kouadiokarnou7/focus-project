...
export const DashboardView: React.FC = () => {
  ...
  // Format total focus time (e.g., 3h 45m or 45m)
  ...
  // Total stroke circumference is 2 * Math.PI * radius = 2 * 3.14159 * 45 = 282.74 -> 283
  const radius = 45;
  const circumference = 2 * Math.PI * radius;
  const progressRatio = timeLeft / totalDuration;
  // Offset decreases as progress ratio decreases, showing countdown
  ...
  const getRingColor = () => {
    if (mode === "focus") return "#ffb596"; // Primary light orange
    if (mode === "short_break") return "#c0c1ff"; // Soft secondary violet
    return "#8dcdff"; // Soft tertiary blue
  };
    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start w-full">
      ...
      <div className="lg:col-span-8 flex flex-col gap-6 w-full">
        ...
        <div className="glass-panel-active rounded-xl p-6 md:p-8 flex flex-col items-center justify-center relative overflow-hidden min-h-[400px]">
          ...
          <div className="relative w-60 h-60 md:w-64 md:h-64 flex items-center justify-center z-10 mb-8 select-none">
            <svg className="absolute inset-0 w-full h-full transform -rotate-90" viewBox="0 0 100 100">
              ...
              <circle
                ...
                strokeLinecap="round"
                style={{
                  filter: `drop-shadow(0 0 5px ${mode === 'focus' ? 'rgba(255,107,26,0.4)' : 'rgba(99,102,241,0.3)'})`
                }}
              />
            </svg>
          ...
          <div className="flex items-center gap-6 z-10">
            ...
            <button
              onClick={toggleTimer}
              className="w-16 h-16 rounded-full bg-primary-container text-white flex items-center justify-center shadow-[0_0_20px_rgba(255,107,26,0.3)] hover:shadow-[0_0_30px_rgba(255,107,26,0.5)] transition-all duration-300 active:scale-95 hover:scale-105"
            >
          ...
          <div className="mt-8 px-4 py-2 rounded-full bg-primary/10 border border-primary/20 flex items-center gap-2 z-10 max-w-full">
            ...
            <span className="text-[11px] font-bold tracking-wider uppercase text-primary truncate max-w-[250px]">
              {activeTask ? `Actif: ${activeTask.name}` : "Aucune tâche sélectionnée"}
            </span>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-4">
          <div className="flex justify-between items-center px-1">
            ...
            <button
              ...
              className="bg-primary-container text-white py-1.5 px-3 rounded-lg shadow-sm hover:brightness-105 transition-all text-xs font-bold flex items-center gap-1 active:scale-95"
            >
              ...
              upcomingTasks.map((task) => (
                ...
                >
                  ...
                  <button
                    ...
                    className="w-6 h-6 rounded-full border border-on-surface-variant/40 group-hover:border-primary flex items-center justify-center transition-all"
                  >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] font-bold tracking-wider uppercase text-on-surface-variant opacity-75 mt-0.5">
                      {task.category} • Priorité {task.priority}
                    </p>
                  </div>
                  <div className="flex items-center gap-1.5 text-xs text-on-surface-variant font-medium bg-surface-container px-2.5 py-1 rounded-md border border-border-glass/40">
                    <span>🍅</span>
                    <span className="font-mono">{task.completedPomodoros}/{task.estimatedPomodoros}</span>
                  </div>
                </div>
              ))
            ) : (
      ...
      <div className="lg:col-span-4 flex flex-col gap-6 w-full">
        ...
        <div className="glass-panel rounded-xl p-6 flex flex-col h-full min-h-[460px]">
          ...
          {/* Bar Chart representing focus hours per day */}
          <div className="flex-1 flex flex-col justify-end gap-3 relative pb-4 min-h-[160px]">
            ...
            <div className="flex items-end justify-between h-36 px-2 z-10">
              ...
              {/* Wed */}
              <div className="flex flex-col items-center gap-2 w-full">
                <div className="w-full max-w-[20px] bg-primary/80 rounded-t-sm h-[90%] shadow-[0_0_10px_rgba(255,107,26,0.3)]"></div>
                <span className="text-[10px] font-bold text-on-surface-variant/50">M</span>
              </div>
              {/* Thu */}
              <div className="flex flex-col items-center gap-2 w-full">
                <div className="w-full max-w-[20px] bg-primary rounded-t-sm h-[60%] shadow-[0_0_15px_rgba(255,107,26,0.5)]"></div>
